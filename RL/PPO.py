...
class Critic(nn.Module):
    def __init__(self, actor_like: nn.Module):
        ...
        if h is None:
            h = getattr(getattr(actor_like, "config", None), "hidden_size", None)
        if h is None:
...
class PPOTrainer:
    def __init__(
        ...
        actor_tokenizer,            # 需有 .decode(ids) 或 .batch_decode
        reward_tokenizer,           # HF tokenizer
        optimizer_actor,
        optimizer_critic,
        device: str = "cuda",
        mb_size_logits: int = 1,    # logits 计算 micro-batch
        mb_size_values: int = 1,    # values 计算 micro-batch
        kl_ctl: float = 0.2,
        ppo_clip: float = 0.2,
    ...
    def evaluate_experience(self, samples: Samples):
        ...
        with torch.no_grad():
            ...
            v_mae = float(((values - returns).abs() * action_mask).sum() /
                          action_mask.sum().clamp_min(1e-8))
    ...
    def train_on_experience(self, exp: Experience, use_token_entropy: bool = False):
        ...
        # clip_frac（基于参与损失的 token）
        with torch.no_grad():
            L = ratio.size(1)
            sel = sel_mask[:, :L].to(ratio.dtype)

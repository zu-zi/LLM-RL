...
class DAPOTrainer:
    ...
    def step_on_groups(self, groups: List[List[dict]], block_size: int) -> Dict[str, float]:
        ...
        lp_resp = (lp_a * mask_t).sum(dim=1) / (denom ** alpha)  # [B]

        # 可选 KL：对参考策略（若存在）
        if self.ref is not None and self.kl_ctl > 0.0:
            with torch.no_grad():
                lp_r = lp_r_full[:, :L]  # type: ignore
                # 使用稳定近似（与 GRPO 中一致的 k3 形式也可，这里直接 forward KL 近似）
                d = (lp_a - lp_r)
                kl_tok = (torch.exp(d) - 1.0 - d) * mask_t
                kl_seq = kl_tok.sum(dim=1) / denom  # [B]
        else:
            kl_seq = None

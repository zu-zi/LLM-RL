# rollout_worker.py
import argparse, os, json, time, uuid, random, glob, sys
...
try:
    ...
    print(f"[worker][warn] sglang import failed: {e}", flush=True)
    sgl = None
...
def _flush_incremental(out_dir: str, stash: List[dict]) -> None:
    ...
    print(f"[worker] flush {os.path.basename(path)}", flush=True)
...
def _load_json_list(path: Optional[str]) -> Optional[List[int]]:
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            arr = json.load(f)
        return arr if isinstance(arr, list) and all(isinstance(x, int) for x in arr) else None
    except Exception as e:
        print(f"[worker][warn] read indices {path}: {e}", flush=True); return None
...
class EngineWrapper:
    def __init__(self, init_path: str, strategy: str, mtime_glob: str):
        self.path = init_path
    ...
    def _load(self, path: str):
        ...
        for attempt in range(3):
            try:
                print(f"[worker] Engine -> {path}", flush=True)
                self.eng = sgl.Engine(model_path=path)
                self.path = path
                self._free_cuda()
                return
            except Exception as e:
                print(f"[worker][warn] init failed ({attempt+1}/3): {e}", flush=True)
    ...
    def _update_signature(self, first=False):
        ...
        if first: print(f"[worker] reload sig init: {self._last_sig}", flush=True)

    def maybe_reload(self) -> bool:
        if self.strategy in ("none", None): return False
        new_sig = self._current_signature()
        if new_sig != self._last_sig:
            print(f"[worker] hot-reload: {self._last_sig} -> {new_sig}", flush=True)
            self._load(self.path); self._last_sig = new_sig
            return True
        return False
...
def main():
    ...
    ap.add_argument("--top-k", type=int, default=0)  # 兼容保留
    ...
    ap.add_argument("--reload-strategy", type=str, default="dir_mtime",
                    choices=["none", "dir_mtime", "realpath"])
    ...
    try:
        ...
        print(f"[worker][fatal] load prompts: {e}", flush=True); sys.exit(0)
    ...
    if args.use_only_train:
        if train_idx_from_file:
            ...
            if isinstance(pb_eval_idx, list) and pb_eval_idx:
                ...
                train_indices = [i for i in range(N) if i not in ev]
            else:
    ...
    # 引擎
    engw = EngineWrapper(init_path=args.model, strategy=args.reload_strategy, mtime_glob=args.mtime_glob)

    total_ok = total_try = 0
    stash: List[dict] = []
    last_flush = time.time()
    batch_counter = 0

    import hashlib
    def _hid(ids: List[int]) -> str:
        h = hashlib.sha1(); h.update((",".join(map(str, ids))).encode("utf-8")); return h.hexdigest()

    seen_hid, seen_pid_resp = set(), set()

    def _bad(resp: str) -> bool:
        s = (resp or "").strip()
        if len(s) < 8: return True
        return "\ufffd" in s  # 有替换符基本是坏样本

    while total_ok < args.count:
        if args.refresh_every_batches > 0 and batch_counter and batch_counter % args.refresh_every_batches == 0:
            try:
                engw.maybe_reload()
            except Exception as e:
                print(f"[worker][warn] hot-reload check: {e}", flush=True)
        ...
        if free_mb < int(args.min_free_mb):
            if mb > 1:
                mb = max(1, mb // 2)
                print(f"[worker] low mem {free_mb}MB -> mb={mb}", flush=True)
            else:
                time.sleep(max(0.1, float(args.sleep_on_lowmem)))

        take = min(mb, args.count - total_ok)
        idxs = random.sample(train_indices, k=take) if len(train_indices) >= take else \
               [train_indices[(total_ok + i) % len(train_indices)] for i in range(take)]
        ...
        for attempt in range(3):
            try:
                ...
                print(f"[worker][warn] generate ({attempt+1}/3): {e}", flush=True)
                backoff = _retry_sleep(backoff)
        if outs is None:
        ...
        for k, out in enumerate(outs):
            ...
            if _bad(resp): continue

            # 截到第一轮
            for m in ["\n\nHuman:", "\nHuman:", "\n\nAssistant:", "\nAssistant:"]:
                pos = resp.find(m)
                if pos != -1: resp = resp[:pos]; break
            resp = resp.lstrip()
    ...
    print(f"[worker] ok={total_ok} try={total_try} acc={acc:.1f}% model={engw.path}", flush=True)
    sys.exit(0)
...
if __name__ == "__main__":
    try:
        ...
        print(f"[worker][fatal] {e}", flush=True); sys.exit(0)
